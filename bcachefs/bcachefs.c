...
const void *benz_bch_next_sibling(const void *p, uint32_t sizeof_p, const void *p_end,
                                  const void *c, struct u64s_spec u64s_spec) {
    if (c == NULL) {
...
const struct bch_sb_field *benz_bch_next_sb_field(const struct bch_sb *      p,
                                                  const struct bch_sb_field *c,
                                                  enum bch_sb_field_type     type) {
    const uint8_t *p_end = (const uint8_t *)p + p->u64s * BCH_U64S_SIZE;
    do {
        c = (const struct bch_sb_field *)benz_bch_next_sibling(p, sizeof(*p), p_end, c,
                                                               U64S_BCH_SB_FIELD);
...
const struct jset_entry *benz_bch_next_jset_entry(const struct bch_sb_field *p, uint32_t sizeof_p,
    ...
    const uint8_t *p_end = (const uint8_t *)p + p->u64s * BCH_U64S_SIZE;
    do {
        c = (const struct jset_entry *)benz_bch_next_sibling(p, sizeof_p, p_end, c,
                                                             U64S_JSET_ENTRY);
    } while (c && type != BCH_SB_FIELD_NR && c->type != type);
    return c;
}
...
const struct bch_val *benz_bch_next_bch_val(const struct bkey *p, const struct bch_val *c,
                                            uint32_t sizeof_c) {
    const struct bch_val *p_end = (const void *)((const uint8_t *)p + p->u64s * BCH_U64S_SIZE);
...
const struct bset *benz_bch_next_bset(const struct btree_node *p, const struct bset *c,
    ...
    do {
        if (c == NULL) {
            c = &p->keys;
            // printf("START BSET %lu %lu\n", (uint64_t)p_end - (uint64_t)c, &p->keys);
        ...
        if (c >= p_end) {
            // printf("STOP BSET %lu %lu\n", (uint64_t)c, (uint64_t)p_end);
            c = NULL;
        }

    } while (c && !c->u64s);

    // printf("BSET (start: %lu) (end: %lu)\n", INT(c), INT(p_end));
    return c;
}

const struct bkey *benz_bch_next_bkey(const struct bset *p, const struct bkey *c,
...
const struct bkey *benz_bch_file_offset_size(const struct bkey *bkey, const struct bch_val *bch_val,
                                             uint64_t *file_offset, uint64_t *offset,
                                             uint64_t *size) {
...
uint64_t benz_bch_fread_btree_node(struct btree_node *btree_node, const struct bch_sb *sb,
                                   const struct bch_extent_ptr *bch_extent_ptr, FILE *fp) {
...
int BCacheFS_iter(const BCacheFS *this, BCacheFS_iterator *iter, enum btree_id type) {
    ...
    if (iter->btree_ptr &&
        !benz_bch_fread_btree_node(iter->btree_node, this->sb, iter->btree_ptr->start, this->fp)) {
        iter->btree_ptr = NULL;
    }
    return iter->jset_entry && iter->btree_node && iter->btree_ptr;
}

int BCacheFS_next_iter(const BCacheFS *this, BCacheFS_iterator *iter,
                       const struct bch_btree_ptr_v2 *btree_ptr) {
    BCacheFS_iterator *next_it = malloc(sizeof(BCacheFS_iterator));
    *next_it                   = (BCacheFS_iterator){.type       = iter->type,
                                   .btree_node = benz_bch_malloc_btree_node(this->sb),
                                   .btree_ptr  = btree_ptr};
    if (next_it->btree_ptr && !benz_bch_fread_btree_node(next_it->btree_node, this->sb,
                                                         next_it->btree_ptr->start, this->fp)) {
...
int BCacheFS_iter_fini(const BCacheFS *this, BCacheFS_iterator *iter) {
    ...
    *iter = (BCacheFS_iterator){
        .type = BTREE_ID_NR, .btree_node = iter->btree_node, .next_it = iter->next_it};
    return iter->next_it == NULL && iter->btree_node == NULL;
}

const struct bch_val *_BCacheFS_iter_next_bch_val(const struct bkey *       bkey,
...
const struct jset_entry *BCacheFS_iter_next_jset_entry(const BCacheFS *this,
                                                       BCacheFS_iterator *iter) {
    const struct jset_entry *  jset_entry = iter->jset_entry;
    const struct bch_sb_field *sb_field_clean =
        (const void *)benz_bch_next_sb_field(this->sb, NULL, BCH_SB_FIELD_clean);

    jset_entry = benz_bch_next_jset_entry(sb_field_clean, sizeof(struct bch_sb_field_clean),
                                          jset_entry, BCH_JSET_ENTRY_btree_root);
...
const struct bch_btree_ptr_v2 *BCacheFS_iter_next_btree_ptr(const BCacheFS *this,
                                                            BCacheFS_iterator *iter) {
    (void)this;
    const struct jset_entry *      jset_entry = iter->jset_entry;
    const struct bch_btree_ptr_v2 *btree_ptr  = iter->btree_ptr;
    if (btree_ptr) {
        btree_ptr = (const void *)benz_bch_next_bch_val(
            &jset_entry->start->k, (const void *)btree_ptr, sizeof(struct bch_btree_ptr_v2));
    } else {
    ...
    while (bch_val && btree_ptr->start->unused) {
        bch_val =
            benz_bch_next_bch_val(&jset_entry->start->k, bch_val, sizeof(struct bch_btree_ptr_v2));
        btree_ptr = (const void *)bch_val;
    }

    printf("Btree root (unused: %d) (offset: %lu)\n", btree_ptr->start->unused,
           btree_ptr->start->offset);
    return btree_ptr;
}
...
BCacheFS_extent BCacheFS_iter_make_extent(const BCacheFS *this, BCacheFS_iterator *iter) {
    ...
    benz_bch_file_offset_size(bkey, iter->bch_val, &extent.file_offset, &extent.offset,
                              &extent.size);
    if (bkey->type == KEY_TYPE_inline_data) {
        extent.offset = benz_bch_inline_data_offset(
            iter->btree_node, iter->bch_val, benz_bch_get_extent_offset(iter->btree_ptr->start));
...
inline uint64_t benz_get_flag_bits(const uint64_t bitfield, uint8_t first_bit, uint8_t last_bit) {
    return bitfield << (sizeof(bitfield) * 8 - last_bit) >>
           (sizeof(bitfield) * 8 - last_bit + first_bit);
}
