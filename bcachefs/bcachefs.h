...
#define BCH_SB_FIELDS()                                                                     \
    x(journal, 0) x(members, 1) x(crypt, 2) x(replicas_v0, 3) x(quota, 4) x(disk_groups, 5) \
        x(clean, 6) x(replicas, 7) x(journal_seq_blacklist, 8)

#define BCH_JSET_ENTRY_TYPES()                                                           \
    x(btree_keys, 0) x(btree_root, 1) x(prio_ptrs, 2) x(blacklist, 3) x(blacklist_v2, 4) \
        x(usage, 5) x(data_usage, 6) x(clock, 7) x(dev_usage, 8)

/* Btree: */

#define BCH_BTREE_IDS()                                                                          \
    x(extents, 0) x(inodes, 1) x(dirents, 2) x(xattrs, 3) x(alloc, 4) x(quotas, 5) x(stripes, 6) \
        x(reflink, 7)
...
#define BCH_BKEY_TYPES()                                                                           \
    x(deleted, 0) x(discard, 1) x(error, 2) x(cookie, 3) x(hash_whiteout, 4) x(btree_ptr, 5)       \
        x(extent, 6) x(reservation, 7) x(inode, 8) x(inode_generation, 9) x(dirent, 10)            \
            x(xattr, 11) x(alloc, 12) x(quota, 13) x(stripe, 14) x(reflink_p, 15) x(reflink_v, 16) \
                x(inline_data, 17) x(btree_ptr_v2, 18) x(indirect_inline_data, 19) x(alloc_v2, 20)
...
#define BCH_KEY_MAGIC                                                                              \
    (((uint64_t)'b' << 0) | ((uint64_t)'c' << 8) | ((uint64_t)'h' << 16) | ((uint64_t)'*' << 24) | \
     ((uint64_t)'*' << 32) | ((uint64_t)'k' << 40) | ((uint64_t)'e' << 48) |                       \
     ((uint64_t)'y' << 56))
...
// Where is the size variable for each struct
...
struct bch_extent_crc32 {
    uint32_t type : 2, _compressed_size : 7, _uncompressed_size : 7, offset : 7, _unused : 1,
        csum_type : 4, compression_type : 4;
    uint32_t csum;
} __attribute__((packed, aligned(8)));
...
struct bch_extent_crc64 {
    uint64_t type : 3, _compressed_size : 9, _uncompressed_size : 9, offset : 9, nonce : 10,
        csum_type : 4, compression_type : 4, csum_hi : 16;
    uint64_t csum_lo;
} __attribute__((packed, aligned(8)));
...
struct bch_extent_crc128 {
    uint64_t type : 4, _compressed_size : 13, _uncompressed_size : 13, offset : 13, nonce : 13,
        csum_type : 4, compression_type : 4;
    struct bch_csum csum;
} __attribute__((packed, aligned(8)));
...
static const struct uuid BCACHE_MAGIC = {0xc6, 0x85, 0x73, 0xf6, 0x4e, 0x1a, 0x45, 0xca,
